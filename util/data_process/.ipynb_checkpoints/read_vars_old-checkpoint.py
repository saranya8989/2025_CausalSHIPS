...
def read_processed_vars_era5(loc=None,foldernames=['era5_wmaxdelv','tigramite_6hr'],year=None,stormname=None,era5_dropvar=None):
    ...
    for foldername in foldernames:
        ...
        file_list = glob.glob(loc+str(foldername)+'/'+str(year)+'/'+str(year)+'*'+str(stormname)+'.csv')
        if file_list:
            tmp = pd.read_csv(file_list[0],delimiter=r",").fillna(0)
        else:
            print(f"No files found for {stormname} in {year}")
        #tmp = _clean_up(tmp)
        
        if foldername=='era5_wmaxdelv':
            tmp = tmp.iloc[:,:]
            tmp = tmp[['delv24']] #change the target name along with config.py here according to the lag setting, 
